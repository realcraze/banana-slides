...
项目在 Docker Hub 提供了构建好的后端镜像（同步主分支最新版本），可以跳过后端本地构建步骤，实现快速部署。前端静态资源由 `frontend-build` 服务导出到宿主机目录，再交给外部 Nginx 托管：
...
- 前端静态资源导出目录：`./frontend-dist`
- 后端 API：http://localhost:5000
- 对外访问入口：请使用外部 Nginx 托管 `./frontend-dist`，并反向代理 `/api`、`/files`、`/health`

4. **导出前端静态资源**

`docker compose up --build -d` 会自动构建并导出前端资源。
...
仓库提供了示例配置文件：[docker/nginx-external.conf.example](/Users/bogao/Documents/repo/banana-slides/docker/nginx-external.conf.example)

- `root` 指向导出的 `frontend-dist` 目录
- `/api`、`/files`、`/health` 反向代理到后端端口
- 后续若接入 `oauth2-proxy`，建议将认证放在外部 Nginx 层完成
